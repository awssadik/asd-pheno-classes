...
def get_main_spark_data_for_GFMM():
    ...
    scqdf = scqdf.set_index('subject_sp_id', drop=True).drop(
        ['respondent_sp_id', 'family_sf_id', 'biomother_sp_id', 
         ...
         'summary_score', 'q01_phrases'], axis=1
    )
    ...
    bhcdf = bhcdf.loc[
        ...
        (bhcdf['age_at_eval_years'] >= 4)
    ]
    bhsdf = bhsdf.loc[
        (bhsdf['age_at_eval_years'] <= 18) & 
        (bhsdf['age_at_eval_years'] >= 4)
    ]
    
    bhcdf = bhcdf.set_index('subject_sp_id', drop=True).drop(
        ['respondent_sp_id', 'family_sf_id', 'biomother_sp_id', 
         'biofather_sp_id', 'sex', 'current_depend_adult', 
         'age_at_eval_months', 'age_at_eval_years'], axis=1
    )
    ...
    cbcl_2 = cbcl_2.set_index('subject_sp_id', drop=True).drop(
        ['respondent_sp_id', 'family_sf_id', 'biomother_sp_id', 
         ...
         'history_social_studies', 'arithmetic_math', 'science'], axis=1
    )
    
    # Convert categorical variables to numerical variables
    cbcl_2 = cbcl_2.replace({
    ...
    clean_df.to_csv('../PhenotypeClasses/data/spark_5392_cohort.txt', sep='\t')
